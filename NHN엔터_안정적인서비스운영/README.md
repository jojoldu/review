...
* 자동 복구
    * 장애시 루틴하게 하는 작업
        * ex: 프로세스 재구동 등을 특저 조건일때 자동으로 수행하도록
        * 대부분의 문제는 데몬을 재 실행하면 해결됨 => 그럼 이렇게 간단한 것도 수동으로 해야할까?
        * StackStorm: 자동화 플랫폼
...
* 동적 쿼리를 사용하지 않는 것을 권장한다
    * 1개의 동적 쿼리는 생각보다 적은 N개의 정적 쿼리로 변경 가능하다
...
* 최저 값 모니터링
    * 데몬 개수의 최대값 뿐만 아니라 최소값도 함께 알람을 설정 필요
* 주기적으로 수치 점검이 필요
...
* 사례 4
    ...
    * 일단 OK 메세지로 처리하고 이후 로그나 기타 후처리로 결제 처리 한다 
        * 보통 이렇게 후처리로 맞추는 방법을 **대사**라고 함 
* 로그
    * Debug 레벨의 로깅: 로그를 껐더니 속도가 10배 향상
* 타임아웃
    * 디폴트 값 사용 주의
