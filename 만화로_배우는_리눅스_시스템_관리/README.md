...
추석 연휴때 회사의 **이중화 작업으로!!!!** 고향인 대구에 내려가지도 못해서 각잡고 다 읽고 정리를 하였다. <br/>
...
- [만화로 배우는 리눅스 시스템 관리 - Prio 지음 / 서수환 옮김 ([Yes24](http://www.yes24.com/24/goods/30705473))](#-prio-yes24httpwwwyes24com24goods30705473)
    - [1화 다른 컴퓨터를 리모트로 조작하고 싶어 (ssh)](#1-ssh)
    - [2화 임시로 관리자 권한을 얻고 싶어 (sudo)](#2-sudo)
    - [3화 다양한 문자열을 한 번에 검색하고 싶어 (grep)](#3-grep)
    - [4화 터미널에서도 대화형으로 파일을 편집하고 싶어 (vim)](#4-vim)
    - [5화 vim에서도 복사 & 붙이기 & 되돌리기를 하고 싶어 (yank)](#5-vim-yank)
    - [6화 갑작스러운 네트워크 끊김에서 복귀하고 싶어 (가상터미널 : tmux)](#6-tmux)
    - [7화 다른 화면도 보면서 작업하고 싶어 (화면 분할)](#7-)
    - [8화 최근 실행한 명령어를 호출하고 싶어 (명령어 이력)](#8-)
    - [9화 명령어 이력 검색](#9-)
    - [10화 네트워크를 건너 파일 복사 (scp)](#10-scp)
    - [11화 시스템 부하를 파악하고 싶어 (top)](#11-top)
    - [12화 시스템 메모리 부족을 파악하고 싶어 (top 표시 전환)](#12-top-)
    - [13화 로그 파일에서 필요한 줄만 뽑고 싶어 (파이프라인 |)](#13-)
    - [14화 작업 절차를 자동화하고 싶어 (쉘 스크립트)](#14-)
    - [15화 같은 문자열을 스크립트에서 재사용하고 싶어 (쉘 변수)](#15-)
    - [16화 작업 환경과 상태를 정해서 스크립트를 실행하고 싶어 (환경 변수)](#16-)
    - [17화 로그 파일에서 필요한 줄만 뽑고 싶어 (cut)](#17-cut)
    - [18화 같은 내용의 줄을 세어보고 싶어 (sort와 uniq)](#18-sort-uniq)
    - [19화 CSV 파일을 열의 내용에 따라 정렬하고 싶어 (sort와 >)](#19-csv-sort-)
    - [20화 명령줄 지정으로 작업 내용을 바꾸고 싶어 (명령줄 인수)](#20-)
    - [21화 조건에 따라 처리 흐름을 바꾸고 싶어 (if)](#21-if)
    - [22화 명령어 이상 종료에 대응하고 싶어 ($?)](#22-)
    - [23화 같은 처리를 반복해서 실행하고 싶어 (for)](#23-for)
    - [24화 공통 처리를 계속 재사용하고 싶어 (쉘 함수)](#24-)
- [총평](#)
...
* 사용법 : ```grep "검색하고 싶은 문자열" 찾을 파일이 있는 디렉토리 경로``` 
